...
namespace ECAStateTreeInternal
{
	...
	UStateTreeEditorData* ResolveEditorData(const FString& Path, FString& OutError)
	{
		...
		// UStateTree::EditorData is a TObjectPtr<UObject> under WITH_EDITORONLY_DATA
		// (the runtime module can't reference the editor-data UClass directly so the field
		// is typed as UObject and we Cast). 5.8 added a static GetEditorData helper but
		// Epic forgot to mark it UE_API in 5.8.0, so the symbol isn't exported from
		// UnrealEditor-StateTreeEditorModule.dll for external linkers — call the static
		// helper from C++ outside that module and you get LNK2019. Use the field cast
		// pattern on both engines.
