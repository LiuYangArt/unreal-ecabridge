...
TSharedPtr<FJsonObject> FECAMCPServer::HandleToolsCall(const TSharedPtr<FJsonObject>& Params)
{
	...
	// MCP spec rev 2025-06-18 §Tools: when a tool advertises an outputSchema,
	// the response MUST also include `structuredContent` conforming to it.
	// Text content above stays for clients that don't read the schema; the
	// chunking cap applies to that text view only — structuredContent always
	// carries the full object.
	if (CommandResult.bSuccess && CommandResult.ResultData.IsValid())
	{
		TSharedPtr<IECACommand> ExecutedCommand = FECACommandRegistry::Get().GetCommand(ToolName);
		if (ExecutedCommand.IsValid() && ExecutedCommand->GetOutputSchema().IsValid())
		{
			Result->SetObjectField(TEXT("structuredContent"), CommandResult.ResultData);
		}
	}
