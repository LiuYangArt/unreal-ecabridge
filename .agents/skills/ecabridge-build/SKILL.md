...
2. For a copied project plugin, compare or sync from the Git source before building. For the HOMEPCWS project, the target is:
   ...
   ```
3. Prefer the bundled project build helper:
   ```powershell
   python .agents\skills\ecabridge-build\scripts\build_ecabridge_project.py --project F:\P4V\liuyang_HOMEPCWS_Env\Paralogue.uproject
   ```
   For source-repo plugin packaging, use:
   ```powershell
...
- Run UnrealBuildTool through `Engine\Build\BatchFiles\Build.bat` for project builds.
- Run UAT `BuildPlugin` through `tools\build_plugin.py` for source-repo plugin packages.
- Temporarily disable known unrelated broken engine plugin descriptors such as `CascLiveLink.uplugin` when they declare missing modules, then always restore them after the build.
...
If UE 5.7 fails before ECABridge compiles with `Unable to find module 'CascLiveLink'`, use the build helper. It treats this as an unrelated broken engine plugin descriptor, disables only the descriptor for the build window, and restores it in `finally`.
